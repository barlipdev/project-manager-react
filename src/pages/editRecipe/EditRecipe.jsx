...
export default class EditRecipe extends Component {
    constructor(props) {
      ...
      componentDidMount(){
          ...
          axios.get("http://51.68.139.166:8091/recipe/"+recipeId,{
            headers: {"Authorization": "Bearer "+ sessionStorage.getItem("token")}
            })
            .then(response => {
                this.setState({ name: response.data.name});
                this.setState({ foodType: response.data.foodType});
                this.setState({ description: response.data.description});
                this.setState({ productsTmp: response.data.productList});
                this.state.productsTmp.forEach(product => {
                    product.id = uuid();
                })
                this.setState({ products: []});
                this.setState({ products: this.state.productsTmp});
            })
            .catch(error => {
                console.log(error);
            })
            ...
            if (this.state.products.length > 0){
                this.state.products.forEach(product => {
                    ...
                    if (product.id === this.state.id){
                        ...
                        product.productTag = this.state.tags;
                        this.setState({ productsTmp: this.state.products});
                        this.setState({ products: []});
                        this.setState({ products: this.state.productsTmp});
                        finded = true;
                        notifySuccess("You successfully update "+this.state.productName+" with count "+ this.state.count +" "+ this.state.productType);
                        return;
                    }
                })
            }
    ...
    render(){
        ...
        const handleEditProduct = (product) => {
            ...
            this.setState({ tags: product.productTag});
            this.setState({ productType: product.productType});
            this.setState({ count: product.count});
        ...
        const columns = [
              ...
              
            {
                ...
                renderCell: (params) => {
                    return (
                        <>
                        <button className="recipeEdit" onClick={()=>handleEditProduct(params.row)}>Edit</button>
        ...
        return (
            <div className="newRecipe">
                <div className="recipeTitleContainer">
                    <h1 className="recipeTitle">Edit recipe</h1>
                    <button onClick={this.handleAddRecipe} className="recipeAddButton">Update Recipe</button>
                 </div>
